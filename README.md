...
## Initial setup
- Once in coder, create a new workspace from the "K8senvironment" template.
- The workspace's name is not important. The default options for CPU, disk size, and memory are fine.
- Open a terminal in the new environment after it builds.
- If you run `kubectl get all` you see should resources related to coder.
...
```
- After that command completes, run `kubectl get all` again. You should see this: `No resources found in coder namespace.`
...
- To access port `30001`:
  - Go back to the workspace page in coder (the one with options for VS Code or Terminal).
